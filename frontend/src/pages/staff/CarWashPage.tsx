  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold">Car Wash Services</h1>
        ...
        <div>
          {bookings.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold mb-2">No Car Wash Bookings</h3>
            ...
            <div className="grid gap-6">
              {bookings.map((booking) => (
                <div
                  key={booking._id}
                  className="bg-card border border-border rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow cursor-pointer"
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-3 py-1 text-sm font-medium rounded-full flex items-center gap-2 ${getStatusColor(booking.status)}`}>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="flex items-center gap-4 text-sm">
                      <span className={`flex items-center gap-1 ${
                        ...
                        Before Photos ({booking.carWash.beforeWashPhotos?.length || 0}/4)
                      </span>
                      <span className={`flex items-center gap-1 ${
                        booking.carWash.afterWashPhotos?.length > 0 ? 'text-green-600' : 'text-muted-foreground'
                      }`}>
                        <CheckCircle className="w-4 h-4" />
                        After Photos ({booking.carWash.afterWashPhotos?.length || 0}/4)
                      </span>
                    </div>
